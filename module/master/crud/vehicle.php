 ...
 **/
    ...
    if($action=='consult'){
        ...
        if($action=='select'){
            ...
            if ($stmt = $mysqli->prepare("SELECT iVehID, cVehPla, cVehTuc, vt.iVehTypID, vt.cVehTypInf, vt.cVehTypNam, vc.iVehClaID, vc.cVehClaInf, vc.cVehClaNam, vy.iVehCatID, vy.cVehCatInf, vy.cVehCatNam, cVehWei, iMeaIDWei, cVehLen, iMeaIDLen, cVehWid, iMeaIDWid, cVehHei, iMeaIDHei, cVehSta FROM tm_vehicle AS v, tm_vehicle_class AS vc, tm_vehicle_type AS vt, tm_vehicle_category AS vy WHERE v.iVehClaID=vc.iVehClaID AND v.iVehTypID=vt.iVehTypID AND v.iVehCatID=vy.iVehCatID")){
                ...
                while($row = $stmt->fetch()) {
                    ...
                    echo '<tr><td><input id="c'.$vehicle_id.'" name="row_sel" type="checkbox" class="row_sel uni_style" data-id="'.$vehicle_id.'"></td>'.
                        ...
                        '<td class="center">'.
                            '<a style="cursor:pointer;" class="edit hint--left" data-hint="Editar" data-id="'.$vehicle_id.'" data-plate="'.$vehicle_plate.'" data-tuc="'.$vehicle_tuc.'" data-type="'.$vehicle_type_id.'" data-class="'.$vehicle_class_id.'" data-category="'.$vehicle_category_id.'" data-weight="'.$measure_weight.'" data-lenght="'.$vehicle_lenght.'" data-width="' .$vehicle_width.'" data-height="'.$vehicle_height.'" data-status="'.$vehicle_status.'"><i class="glyphicon glyphicon-edit" /></a>'.                            
                        '</td></tr>';
                }
            }
        }else{
           ...
           if($action=='insert'){
               //echo "INSERT INTO tm_vehicle (cVehPla, cVehTuc, iVehTypID, iVehClaID, iVehCatID, cVehWei, iMeaIDWei, cVehLen, iMeaIDLen, cVehWid, iMeaIDWid, cVehHei, iMeaIDHei, cVehSta) VALUES "

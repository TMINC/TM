 ...
 **/
    ...
    if($action=='consult'){
        ...
        if($action=='select'){
            ...
            if ($stmt = $mysqli->prepare("SELECT iVehID, cVehPla, cVehTuc, vt.iVehTypID, vt.cVehTypInf, vt.cVehTypNam, vc.iVehClaID, vc.cVehClaInf, vc.cVehClaNam, cVehWei, vy.iVehCatID, vy.cVehCatInf, vy.cVehCatNam, iMeaIDWei, cVehLen, cVehWid, cVehHei, iMeaIDHei, cVehSta FROM tm_vehicle AS v, tm_vehicle_class AS vc, tm_vehicle_type AS vt, tm_vehicle_category AS vy WHERE v.iVehClaID=vc.iVehClaID AND v.iVehTypID=vt.iVehTypID AND v.iVehCatID=vy.iVehCatID")){
                ...
                while($row = $stmt->fetch()) {
                    ...
                    echo '<tr><td><input id="c'.$vehicle_id.'" name="row_sel" type="checkbox" class="row_sel uni_style" data-id="'.$vehicle_id.'"></td>'.
                        ...
                        '<td class="center">'.
                            '<a style="cursor:pointer;" class="edit hint--left" data-hint="Editar" data-id="'.$vehicle_id.'" data-plate="'.$vehicle_plate.'" data-tuc="'.$vehicle_tuc.'" data-type="'.$vehicle_type_id.'" data-class="'.$vehicle_class_id.'" data-weight="'.$vehicle_weight.'" data-category="'.$vehicle_category_id.'" data-measure_weight="'.$vehicle_measure_weight.'" data-length="'.$vehicle_length.'" data-width="' .$vehicle_width.'" data-height="'.$vehicle_height.'" data-measure_height="'.$vehicle_measure_height.'" data-status="'.$vehicle_status.'"><i class="glyphicon glyphicon-edit" /></a>'.                            
                        '</td></tr>';
                }
            }
        }else{
           ...
           if($action=='insert'){
               echo "INSERT INTO tm_vehicle (cVehPla, cVehTuc, iVehTypID, iVehClaID, cVehWei, iVehCatID, 1MeaIDWei, cVehLen, cVehWid, cVehHei, iMeaIDHei, cVehSta) VALUES "
                       . "('".$vehicle_plate."', '".$vehicle_tuc."', '".$vehicle_type_id."', '".$vehicle_class_id."', '".$vehicle_weight."', '".$vehicle_category_id."' '".$vehicle_measure_weight."', '".$vehicle_length."', '".$vehicle_width."', '".$vehicle_height."', '".$vehicle_measure_height."', '".$vehicle_status."')";
               $mysqli->query("INSERT INTO tm_vehicle (cVehPla, cVehTuc, iVehTypID, iVehClaID, cVehWei, iVehCatID, iMeaIDWei, cVehLen, cVehWid, cVehHei, iMeaIDHei, cVehSta) VALUES "
                       . "('".$vehicle_plate."', '".$vehicle_tuc."', '".$vehicle_type_id."', '".$vehicle_class_id."', '".$vehicle_weight."', '".$vehicle_category_id."', '".$vehicle_measure_weight."', '".$vehicle_length."', '".$vehicle_width."', '".$vehicle_height."', '".$vehicle_measure_height."', '".$vehicle_status."')");
           } 
           if($action=='update'){
               echo "UPDATE tm_vehicle SET cVehPla='".$vehicle_plate."' , cVehGen='".$vehicle_tuc."', iVehTypID='".$vehicle_type_id."', iVehClaID='".$vehicle_class_id."', cVehWei='".$vehicle_weight."', iVehCatID='".$vehicle_category_id."' iMeaIDWei='".$vehicle_measure_weight."', cVehLen='".$vehicle_length."', cVehWid='".$vehicle_width."', cVehHei='".$vehicle_height."', iMeaIDHei='".$vehicle_measure_height."', cVehSta='".$vehicle_status."' WHERE iVehID='".$vehicle_id."'";

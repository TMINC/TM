 ...
 **/
    ...
    if($action=='consult'){
        ...
        if($action=='select'){
            ...
            if ($stmt = $mysqli->prepare("SELECT iVehID, cVehPla, cVehTuc, vt.iVehTypID, vt.cVehTypInf, vt.cVehTypNam, vc.iVehClaID, vc.cVehClaInf, vc.cVehClaNam, vy.iVehCatID, vy.cVehCatInf, vy.cVehCatNam, vg.iVehGroID, vg.cVehGroNam, vg.cVehGroDes, cVehWei, iMeaIDWei, cVehLen, iMeaIDLen, cVehWid, iMeaIDWid, cVehHei, iMeaIDHei, cVehSta FROM tm_vehicle AS v, tm_vehicle_class AS vc, tm_vehicle_type AS vt, tm_vehicle_category AS vy, tm_vehicle_group AS vg WHERE v.iVehClaID=vc.iVehClaID AND v.iVehTypID=vt.iVehTypID AND v.iVehCatID=vy.iVehCatID AND v.iVehGroID=vg.iVehGroID")){
                $stmt->execute();
                $stmt->store_result();
                $stmt->bind_result($vehicle_id, $vehicle_plate, $vehicle_tuc, $vehicle_type_id, $vehicle_type_info, $vehicle_type_name, $vehicle_class_id, $vehicle_class_info, $vehicle_class_name, $vehicle_category_id, $vehicle_category_info, $vehicle_category_name, $vehicle_group_id, $vehicle_group_name, $vehicle_group_designation, $vehicle_weight, $measure_weight_id, $vehicle_lenght, $measure_lenght_id, $vehicle_width, $measure_width_id, $vehicle_height, $measure_height_id, $vehicle_status);
                while($row = $stmt->fetch()) {
           ...
           $vehicle_group_id = $_POST['group_id'];
           ...
           if($action=='insert'){              
               $mysqli->query("INSERT INTO tm_vehicle (cVehPla, cVehTuc, iVehTypID, iVehClaID, iVehCatID, iVehGroID, cVehWei, iMeaIDWei, cVehLen, iMeaIDLen, cVehWid, iMeaIDWid, cVehHei, iMeaIDHei, cVehSta) VALUES "
                       . "('".$vehicle_plate."', '".$vehicle_tuc."', '".$vehicle_type_id."', '".$vehicle_class_id."', '".$vehicle_category_id."', '".$vehicle_group_id."', '".$vehicle_weight."', '".$measure_weight_id."', '".$vehicle_lenght."', '".$measure_lenght_id."', '".$vehicle_width."', '".$measure_width_id."', '".$vehicle_height."', '".$measure_height_id."', '".$vehicle_status."')");
           } 
           if($action=='update'){ 
               $mysqli->query("UPDATE tm_vehicle SET cVehPla='".$vehicle_plate."', cVehTuc='".$vehicle_tuc."', iVehTypID='".$vehicle_type_id."', iVehClaID='".$vehicle_class_id."', iVehCatID='".$vehicle_category_id."', iVehGroID='".$vehicle_group_id."', cVehWei='".$vehicle_weight."', iMeaIDWei='".$measure_weight_id."', cVehLen='".$vehicle_lenght."', iMeaIDLen='".$measure_lenght_id."', cVehWid='".$vehicle_width."', iMeaIDWid='".$measure_width_id."', cVehHei='".$vehicle_height."', iMeaIDHei='".$measure_height_id."', cVehSta='".$vehicle_status."' WHERE iVehID='".$vehicle_id."'");
           }
           if($action=='delete'){

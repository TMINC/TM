 ...
 **/
    ...
    if($action=='consult'){
        ...
        if($action=='select'){            
            if ($stmt = $mysqli->prepare("SELECT iDriID, cDriLic, cDriFirNam, cDriLasNam, cDriAdd, cDriPho, cDriDat, cDriDni, cDriBlo, cDriTyp, c.iCarID, c.cCarNam, cDriSta FROM tm_driver AS d, tm_carrier AS c WHERE d.iCarID=c.iCarID")){
                $stmt->execute();
                $stmt->store_result();

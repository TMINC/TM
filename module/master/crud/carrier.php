            ...
            if ($stmt = $mysqli->prepare("SELECT iCarID, cCarNam, cCarAdd, cCarPhoNam, cCarEmaNam, cCarRuc, cCarAge, cCarPhoAge, cCarEmaAge, cCarCon, cCarPhoCon, cCarEmaCon, cCarSta FROM tm_carrier")){
                ...
                while($row = $stmt->fetch()) {
                    ...
                    echo '<tr><td><input id="c'.$carrier_id.'" name="row_sel" type="checkbox" class="row_sel uni_style" data-id="'.$carrier_id.'"></td>'.
                        ...
                        '<td>'.$carrier_name.'<a style="cursor:help;float:right;" class="pop_over hint--right hint--info" data-hint="Detalle" data-content="&perp; '.$carrier_address.'    &phone;'.$carrier_phone_name.'    @&nbsp;'.$carrier_email_name.'" title="'.$carrier_name.'" data-placement="left"><i class="glyphicon glyphicon-briefcase"/></a></td>'.
                        '<td>'.$carrier_ruc.'</td>'.

 ...
 **/
    ...
    if($action=='consult'){
        ...
        if($action=='select'){
            if ($stmt = $mysqli->prepare("SELECT iVehCatID, cVehCatInf, cVehCatNam, cVehCatSta FROM tm_vehicle_category")){
                $stmt->execute();
                $stmt->store_result();
                $stmt->bind_result($category_id, $category_info, $category_name, $category_status);
                while($row = $stmt->fetch()) {
                    ...
                    echo '<tr><td class="center"><input id="c'.$category_id.'" name="row_sel" type="checkbox" class="row_sel uni_style" data-id="'.$category_id.'"></td>'.
                        ...
                        '<td>'.$category_name.'</td>'.
                        '<td class="center">'.
                            '<a style="cursor:pointer;" class="edit hint--left" data-hint="Editar" data-id="'.$category_id.'" data-info="'.$category_info.'" data-name="'.$category_name.'" data-status="'.$category_status.'"><i class="glyphicon glyphicon-edit" /></a>'.                            
                        '</td></tr>';
                }
            }
        }else{
           ...
           if($action=='insert'){
               $mysqli->query("INSERT INTO tm_vehicle_category (cVehCatInf, cVehCatNam, cVehCatSta) VALUES ('".$category_info."', '".$category_name."', '".$category_status."')");
           } 
           if($action=='update'){
               $mysqli->query("UPDATE tm_vehicle_category SET cVehCatInf='".$category_info."' , cVehCatNam='".$category_name."', cVehCatSta='".$category_status."' WHERE iVehCatID='".$category_id."'");
           }
           if($action=='delete'){

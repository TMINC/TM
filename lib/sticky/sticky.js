...
;(function($) {
	...
	$.sticky = function(note, options, callback, type) { return $.fn.sticky(note, options, callback, type); };
	$.fn.sticky = function(note, options, callback, type) {
            var settings ={
            ...
            if(type == 'modal'){
                ...
                if(!$('body').find('.sticky-queue-modal.'+settings.position).html()) {
                    $('body').append('<div class="sticky-queue-modal ' + settings.position + '"></div>');
                }
            }else{
                // Make sure the sticky queue exists
            ...
            if(display) {
                ...
                if(type == 'modal'){
                    $('.sticky-queue-modal.'+settings.position).prepend('<div class="sticky border-' + settings.position + ' ' + settings.type +'" id="' + uniqID + '"></div>');
                }else{
            ...
            $('.sticky').ready(function() {
                ...
                if(settings['autoclose']) {
                    $('#' + uniqID).delay(settings['autoclose']).slideUp(settings['speed'], function(){
                        if(type == 'modal'){
                            var closest = $(this).closest('.sticky-queue-modal');
                        }else{
